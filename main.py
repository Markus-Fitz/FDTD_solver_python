    ...
    for i in range(fields.nx + 1):
        for j in range(fields.ny):
            for k in range(fields.nz):
                ...
                B_x_new = fields.Bx[i, j, k] - dt * (
                    (fields.Ey[i, j, k + 1] - fields.Ey[i, j, k]) / fields.dz
                    - (fields.Ez[i, j + 1, k] - fields.Ez[i, j, k]) / fields.dy
                )
    ...
    for i in range(fields.nx):
        for j in range(fields.ny + 1):
            for k in range(fields.nz):
                ...
                B_y_new = fields.By[i, j, k] - dt * (
                    (fields.Ez[i + 1, j, k] - fields.Ez[i, j, k]) / fields.dx
                    - (fields.Ex[i, j, k + 1] - fields.Ex[i, j, k]) / fields.dz
                )
    ...
    for i in range(fields.nx):
        for j in range(fields.ny):
            for k in range(fields.nz + 1):
                ...
                B_z_new = fields.Bz[i, j, k] - dt * (
                    (fields.Ex[i, j + 1, k] - fields.Ex[i, j, k]) / fields.dy
                    - (fields.Ey[i + 1, j, k] - fields.Ey[i, j, k]) / fields.dx
                )
    ...
    for i in range(fields.nx):
        for j in range(1, fields.ny):
            for k in range(1, fields.nz):
                ...
                E_x_new = fields.Ex[i, j, k] + (dt / (mu0 * eps0)) * (
                    (fields.By[i, j, k] - fields.By[i, j, k - 1]) / fields.dz
                    - (fields.Bz[i, j, k] - fields.Bz[i, j - 1, k]) / fields.dy
                )  # - (dt/eps0)*J_x[i, j, k]
    ...
    for i in range(1, fields.nx):
        for j in range(fields.ny):
            for k in range(1, fields.nz):
                ...
                E_y_new = fields.Ey[i, j, k] + (dt / (mu0 * eps0)) * (
                    (fields.Bz[i, j, k] - fields.Bz[i - 1, j, k]) / fields.dx
                    - (fields.Bx[i, j, k] - fields.Bx[i, j, k - 1]) / fields.dz
                )  # - (dt/eps0)*J_y[i, j, k]
    ...
    for i in range(1, fields.nx):
        for j in range(1, fields.ny):
            for k in range(fields.nz):
                ...
                E_z_new = fields.Ez[i, j, k] + (dt / (mu0 * eps0)) * (
                    (fields.Bx[i, j, k] - fields.Bx[i, j - 1, k]) / fields.dy
                    - (fields.By[i, j, k] - fields.By[i - 1, j, k]) / fields.dx
                )  # - (dt/eps0)*J_z[i, j, k]
...
def calc_Bx_exp(fields):
    for i in range(fields.nx):
        for j in range(fields.ny):
            for k in range(fields.nz):
                fields.Bx_exp[i, j, k] = (
                    fields.Bx[i, j, k] + fields.Bx[i + 1, j, k]
                ) / 2.0
...
def calc_By_exp(fields):
    for i in range(fields.nx):
        for j in range(fields.ny):
            for k in range(fields.nz):
                fields.By_exp[i, j, k] = (
                    fields.By[i, j, k] + fields.By[i, j + 1, k]
                ) / 2.0
...
def calc_Bz_exp(fields):
    for i in range(fields.nx):
        for j in range(fields.ny):
            for k in range(fields.nz):
                fields.Bz_exp[i, j, k] = (
                    fields.Bz[i, j, k] + fields.Bz[i, j, k + 1]
                ) / 2.0
